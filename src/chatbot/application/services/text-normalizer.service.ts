...
export class TextNormalizerService {
  ...
  applySynonyms(text: string): string {
    ...
    const sortedSynonyms = Object.entries(SYNONYMS).sort(
   ...
   */
  hasNegationBefore(text: string, keyword: string): boolean {
  ...
  isNegation(text: string): boolean {
    ...
    return negationStarters.some(
      (starter) =>
        normalizedText.startsWith(starter) || normalizedText === "no"
    );
  }
