...
class IntentDetectorService {
    ...
    constructor(normalizer) {
        this.normalizer = normalizer || new text_normalizer_service_1.TextNormalizerService();
    }
    detect(message) {
        ...
        for (const [intent, keywords] of Object.entries(INTENT_KEYWORDS)) {
            ...
            const matchedKeywords = keywords.filter((kw) => normalizedMessage.includes(kw) || enhancedMessage.includes(kw));
            // Calcular score con peso de la intención
            const weight = INTENT_WEIGHTS[intent] || 0.3;
        ...
        if (bestIntent === "SALE_INTEREST" && isNegated) {
            ...
            const hasNegatedSale = saleKeywords.some((kw) => this.normalizer.hasNegationBefore(message, kw));
            if (hasNegatedSale) {
     ...
     */
    detectQuestionTopic(normalizedMessage, enhancedMessage) {
        for (const [topic, keywords] of Object.entries(QUESTION_TOPIC_KEYWORDS)) {
            const hasMatch = keywords.some((kw) => normalizedMessage.includes(kw) || enhancedMessage.includes(kw));
            if (hasMatch) {
                return topic;
            }
        }
     ...
     */
    detectWithContext(message, previousMessages = []) {
