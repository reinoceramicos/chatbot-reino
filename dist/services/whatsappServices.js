...
const postToWhatsApp = (phoneNumberId, payload) => new Promise((resolve, reject) => {
    ...
    const req = https_1.default.request({
        ...
        res.on("end", () => {
            ...
            if (!ok) {
                return reject(new Error(`WhatsApp API error ${res.statusCode || "NA"}: ${body}`));
            }
            try {
...
const sendTextMessage = async ({ to, body, phoneNumberId = DEFAULT_PHONE_NUMBER_ID, previewUrl = false, }) => {
    if (!to)
        throw new Error("Parametro 'to' es requerido");
    if (!body)
        throw new Error("Parametro 'body' es requerido");
    const payload = {
